...
pub struct BskyConfig {
    /// PDS used for outbound calls when a viewer's profile doesn't advertise
    /// one (rare). Defaults to bsky.social.
...
impl BskyConfig {
    fn from_env() -> Self {
        Self {
            default_pds: env::var("BSKY_DEFAULT_PDS")
                .ok()
...
pub struct AppConfig {
    ...
    /// Origin allowed to embed this plugin in an iframe (RoleLogic dashboard).
