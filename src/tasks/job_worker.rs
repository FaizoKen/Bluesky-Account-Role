//! Job-polling worker. N worker tasks (`WORKER_CONCURRENCY`) run in parallel.
...
async fn dispatch(job: &Job, state: &AppState) -> Result<(), JobError> {
    ...
    match kind {
        ...
        JobKind::AccountSync => {
            let did = job
                .payload
                .get("bsky_account_did")
                .and_then(Value::as_str)
                .ok_or_else(|| {
                    JobError::Terminal("account_sync missing bsky_account_did".into())
                })?;
            // First refresh the broadcaster's followers + lists + starter
            // packs (this is where the AT-Protocol calls happen), then fan
            // out a per-role-link sync to every bound role link.
            crate::tasks::reconcile::refresh_account(state, did).await?;
            sync::sync_for_account(did, state).await?;
        }
    }
    Ok(())
}
