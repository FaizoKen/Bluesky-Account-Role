//! At-rest encryption for sensitive secrets (Bluesky session JWTs).
//!
//! Threat model: an attacker who exfiltrates the database (a dump, a stolen
//! backup, a SQL-injection that reads BYTEA columns) cannot impersonate a
//! broadcaster against Bluesky without also stealing `SESSION_SECRET` from
//! the plugin's environment. This is a meaningful defense-in-depth boundary;
//! it does NOT protect against an attacker with full host access.
//!
//! Construction:
//!   * KEK = HKDF-SHA256(SESSION_SECRET, salt=fixed, info="bluesky-account-role/v1/kek")
...
const HKDF_INFO: &[u8] = b"bluesky-account-role/v1/kek";
const HKDF_SALT: &[u8] = b"bluesky-account-role/v1/salt";
const AAD: &[u8] = b"bluesky-account-role/v1/session-jwt";
...
mod tests {
    ...
    fn round_trip_real_token_size() {
        let pt = b"eyJraWQiOiJ4Iiwic28iOiJiYXIiLCJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.payload.sig";
        let ct = encrypt(SECRET, pt);
        assert_ne!(&ct, pt);
        let decrypted = decrypt(SECRET, &ct).unwrap();
