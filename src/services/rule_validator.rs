...
pub struct RuleTreeBody {
    /// Optional Bluesky-account binding. `None` lets admins save a draft rule
    /// before they've connected a broadcaster — the role grants to nobody
    /// until an account is bound.
    #[serde(default)]
    pub bsky_account_did: Option<String>,
    #[serde(default)]
...
pub fn parse_rule_tree(body: RuleTreeBody) -> Result<ParsedRule, AppError> {
    if !body.grant_on_any_relation {
        if body.groups.is_empty() {
            return Err(AppError::BadRequest(
                "Add at least one OR-group, or pick \"any linked viewer\".".into(),
            ));
        }
    ...
    let bsky_account_did = body.bsky_account_did.and_then(|s| {
        let t = s.trim();
        if t.is_empty() {
            None
        } else {
...
fn validate_condition(
    ...
    // List / starter-pack membership: only `Eq` makes sense (value is the URI).
    use ConditionTarget::*;
    if matches!(target, IsOnList | IsOnStarterPack) && operator != ConditionOperator::Eq {
        return Err(AppError::BadRequest(format!(
            "{where_}: '{}' only supports the 'equals' operator (the value is the list/pack URI).",
            target.as_str()
        )));
    }
    ...
    // List / pack URIs must start with "at://" — a misclick on the dropdown
    // would otherwise silently grant the role to nobody.
    if matches!(target, IsOnList | IsOnStarterPack) {
        let v = value.as_str().unwrap_or("");
        if !v.starts_with("at://") {
            return Err(AppError::BadRequest(format!(
                "{where_}: value must be an at:// URI (pick a list / starter pack from the dropdown)."
            )));
        }
    }
...
mod tests {
    ...
    fn one_group(conds: Vec<ConditionInput>) -> RuleTreeBody {
        RuleTreeBody {
            bsky_account_did: Some("did:plc:test".into()),
    ...
    #[test]
    fn rejects_is_on_list_non_at_uri() {
        let body = one_group(vec![input("is_on_list", "eq", json!("not-a-uri"))]);
        assert!(matches!(
            parse_rule_tree(body),
            Err(AppError::BadRequest(_))
        ));
    ...
    #[test]
    fn accepts_is_on_list_valid_uri() {
        let body = one_group(vec![input(
            "is_on_list",
            "eq",
            json!("at://did:plc:owner/app.bsky.graph.list/abc"),
        )]);
        assert!(parse_rule_tree(body).is_ok());
    }
    ...
    fn bool_coerces_from_string() {
        let body = one_group(vec![input("is_mutual", "eq", json!("true"))]);
        let parsed = parse_rule_tree(body).unwrap();
        assert_eq!(
    ...
    #[test]
    fn handle_domain_in_csv() {
        let body = one_group(vec![input(
            "handle_domain",
            "in",
            json!("bsky.social, example.com"),
        )]);
        let parsed = parse_rule_tree(body).unwrap();
        assert_eq!(
            parsed.rule_tree.groups[0].conditions[0].value,
    ...
    fn regex_pattern_must_compile() {
        let body = one_group(vec![input("handle", "regex", json!("((bad"))]);
        assert!(matches!(
            parse_rule_tree(body),
            Err(AppError::BadRequest(_))
        ));
