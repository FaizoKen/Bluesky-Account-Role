...
//! dashboard embeds our role-config page in an iframe, plus mint/verify the
//! follow-on iframe-session token used on subsequent XHRs from the iframe.
...
// embedded in the rendered role-config page, sent as `Authorization:
// Bearer …` on every subsequent XHR from the iframe.
//
// Bound to (discord_id, guild_id, role_id). Signed with `session_secret`.
// Format: `ifs:{discord_id}:{guild_id}:{role_id}:{exp}:{hmac_hex}`
// -------------------------------------------------------------------------
...
mod tests {
    ...
    const PLUGIN_AUD: &str = "https://plugin.example/bluesky-account-role";
