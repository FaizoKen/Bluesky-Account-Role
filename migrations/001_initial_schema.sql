...
-- Until an admin picks a Bluesky account and sets eligibility,
-- rule_tree = '{"grant_on_any_relation":false,"groups":[]}' AND
-- grant_on_any_relation = FALSE means "grant to nobody" (Convention 42).
--
-- `bsky_account_did` is nullable because POST /register fires the moment the
-- admin creates a role link in the RoleLogic dashboard — well before they
-- have a chance to open the iframe and connect a Bluesky account.
--
...
CREATE INDEX IF NOT EXISTS idx_role_links_account
    ON role_links (bsky_account_did)
    WHERE bsky_account_did IS NOT NULL;

-- Role assignments: local mirror of who currently has which Discord role.
-- Source of truth is RoleLogic; we keep this table to diff against when
-- computing add/remove deltas during sync. CASCADE keeps it in sync with
-- role_links automatically on DELETE /config.
